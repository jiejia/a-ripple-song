...
class Tags_Cloud_Widget extends WP_Widget {
                ...
                <div class="card-body p-4">
                    ...
                    <div class="text-center py-8">
                        <div class="text-base-content/50">
                            ...
                            <p class="text-sm font-medium"><?php _e('No tags yet', 'sage'); ?></p>
                            <p class="text-xs mt-1"><?php _e('Tags will appear here after publishing articles with tags', 'sage'); ?></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    public function form($instance) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of tags:', 'sage'); ?></label>
            <input class="tiny-text" 
                   ...
                   size="3">
            <small class="description"><?php _e('Maximum number of tags to display', 'sage'); ?></small>
        </p>
        
        <p>
            <label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e('Order by:', 'sage'); ?></label>
            <select class="widefat" 
                    id="<?php echo $this->get_field_id('orderby'); ?>" 
                    name="<?php echo $this->get_field_name('orderby'); ?>">
                <option value="count" <?php selected($orderby, 'count'); ?>><?php _e('Post Count', 'sage'); ?></option>
                <option value="name" <?php selected($orderby, 'name'); ?>><?php _e('Tag Name', 'sage'); ?></option>
                <option value="term_id" <?php selected($orderby, 'term_id'); ?>><?php _e('Tag ID', 'sage'); ?></option>
                <option value="rand" <?php selected($orderby, 'rand'); ?>><?php _e('Random', 'sage'); ?></option>
            </select>
        </p>
        
        <p>
            <label for="<?php echo $this->get_field_id('order'); ?>"><?php _e('Sort order:', 'sage'); ?></label>
            <select class="widefat" 
                    id="<?php echo $this->get_field_id('order'); ?>" 
                    name="<?php echo $this->get_field_name('order'); ?>">
                <option value="DESC" <?php selected($order, 'DESC'); ?>><?php _e('Descending (High to Low/Z to A)', 'sage'); ?></option>
                <option value="ASC" <?php selected($order, 'ASC'); ?>><?php _e('Ascending (Low to High/A to Z)', 'sage'); ?></option>
            </select>
        </p>
        <?php
    }
