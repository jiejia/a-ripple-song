...
use function App\Feeds\add_action;
use function App\Feeds\add_feed;
use function App\Feeds\add_filter;
use function App\Feeds\add_query_arg;
use function App\Feeds\apply_filters;
use function App\Feeds\attachment_url_to_postid;
use function App\Feeds\current_user_can;
use function App\Feeds\esc_attr;
use function App\Feeds\esc_html;
use function App\Feeds\esc_url;
use function App\Feeds\esc_url_raw;
use function App\Feeds\flush_rewrite_rules;
use function App\Feeds\get_avatar_url;
use function App\Feeds\get_option;
use function App\Feeds\get_permalink;
use function App\Feeds\get_post_meta;
use function App\Feeds\get_post_time;
use function App\Feeds\get_post_type;
use function App\Feeds\get_site_icon_url;
use function App\Feeds\get_the_excerpt;
use function App\Feeds\get_the_ID;
use function App\Feeds\get_the_title;
use function App\Feeds\get_userdata;
use function App\Feeds\home_url;
use function App\Feeds\is_admin;
use function App\Feeds\is_feed;
use function App\Feeds\nocache_headers;
use function App\Feeds\post_type_exists;
use function App\Feeds\remove_all_actions;
use function App\Feeds\sanitize_text_field;
use function App\Feeds\status_header;
use function App\Feeds\update_option;
use function App\Feeds\url_to_postid;
use function App\Feeds\wp_get_attachment_url;
use function App\Feeds\wp_parse_url;
use function App\Feeds\wp_reset_postdata;
use function App\Feeds\wp_strip_all_tags;
 ...
 *
 * Ported from the previous plugin implementation and adapted for this theme.
 *
 * @package    ARippleSong
 */
class Podcast extends FeedAbstract
{
    /**
    ...
    /**
     * Register the podcast feed endpoint and routing safeguards.
     *
     * @return void
     */
    public function registerHooks(): void
    {
        ...
        add_action('pre_get_posts', array($this, 'fixPodcastArchiveQuery'), 1);
        add_action('template_redirect', array($this, 'preventPodcastSlugFromRenderingFeed'), 0);
        add_action('admin_init', array($this, 'maybeFlushRewriteRules'));
        add_action('send_headers', array($this, 'forcePodcastFeedHeaders'), 0);
        add_filter('redirect_canonical', array($this, 'preventCanonicalRedirectForPodcastFeed'), 10, 2);
    }
    /**
    ...
    {
        add_feed($this->slug(), array($this, 'renderFeed'));
    }
    /**
    ...
    public function fixPodcastArchiveQuery($query)
    {
        ...
        if (post_type_exists($this->postTypeSlug())) {
            $query->is_post_type_archive = \true;
            $query->is_archive = \true;
            $query->set('post_type', $this->postTypeSlug());
        }
    }
    /**
    ...
    /**
     * Return the episode custom post type slug used by this plugin version.
     *
     * @return string
     */
    ...
    public function forcePodcastFeedHeaders()
    {
        ...
        header('Content-Type: application/rss+xml; charset=UTF-8', \true);
        header('X-Content-Type-Options: nosniff', \true);
    }
    /**
    ...
    public function preventPodcastSlugFromRenderingFeed()
    {
        ...
        if (post_type_exists($this->postTypeSlug())) {
            $wp_query->is_post_type_archive = \true;
            $wp_query->is_archive = \true;
            $wp_query->set('post_type', $this->postTypeSlug());
    ...
    {
        if (in_array((string) $key, array('members', 'guests', 'episode_soundbites'), \true)) {
            $value = get_post_meta((int) $post_id, Episode::storedFieldKey((string) $key), \true);
    ...
    private function buildPersonEntries($rawIds, $role)
    {
        ...
        foreach ($ids as $user_id) {
            ...
            $people[] = array('name' => $name, 'role' => $role, 'href' => $user->user_url ? $user->user_url : '', 'img' => get_avatar_url($user_id, array('size' => 300)) ? get_avatar_url($user_id, array('size' => 300)) : '');
        }
        return $people;
    }
    /**
    ...
    private function normalizeItunesExplicit($value, $fallback = 'false')
    {
        ...
        if (in_array($normalized, array('yes', 'true', '1', 'explicit'), \true)) {
            return 'true';
        }
        if (in_array($normalized, array('no', 'false', '0', 'clean'), \true)) {
            return 'false';
        }
        return $fallback;
    }
    ...
    {
        $text = wp_strip_all_tags((string) $text, \true);
        $text = html_entity_decode($text, \ENT_QUOTES | \ENT_HTML5, 'UTF-8');
        $text = preg_replace('/\s+/u', ' ', $text) ?? $text;
        $text = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u', '', $text) ?? $text;
        $text = str_replace(array('**', '__'), '', $text);
        $text = preg_replace('/\s*(?:\[\s*)?(?:&hellip;|…|\.{3})(?:\s*\])?\s*(?:continued|continue\s+reading|read\s+more|\x{7EE7}\x{7EED}\x{9605}\x{8BFB}|\x{7E7C}\x{7E8C}\x{95B1}\x{8B80}|\x{9605}\x{8BFB}\x{66F4}\x{591A})\s*$/iu', '', $text) ?? $text;
        $text = preg_replace('/\s*(?:continued|continue\s+reading|read\s+more|\x{7EE7}\x{7EED}\x{9605}\x{8BFB}|\x{7E7C}\x{7E8C}\x{95B1}\x{8B80}|\x{9605}\x{8BFB}\x{66F4}\x{591A})\s*$/iu', '', $text) ?? $text;
        return trim($text);
    }
    /**
    ...
    public function renderFeed(): void
    {
        ...
        if (!empty($query->posts)) {
            ...
            $latest_ts = (int) get_post_time('U', \true, $latest_post_id, \false);
            if ($latest_ts > 0) {
    ...
    <channel>
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                $pub_ts = (int) get_post_time('U', \true, $post_id, \false);
            ...
            <?php
                if (is_array($episode_soundbites) && !empty($episode_soundbites)) {
                    foreach ($episode_soundbites as $soundbite) {
                        ...
                        $start = isset($soundbite[Episode::fieldKey('start_time')]) ? (float) $soundbite[Episode::fieldKey('start_time')] : null;
                        $dur = isset($soundbite[Episode::fieldKey('duration')]) ? (float) $soundbite[Episode::fieldKey('duration')] : null;
