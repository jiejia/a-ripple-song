    ...
    @if(empty($tags))
      <div class="text-center py-8">
        <div class="text-base-content/50">
          ...
          <p class="text-sm font-medium">{{ __('No tags yet', 'sage') }}</p>
          <p class="text-xs mt-1">{{ __('Tags will appear here after publishing articles with tags', 'sage') }}</p>
        </div>
      </div>
    @else
        ...
        @foreach($tags as $tag)
          <li>
            <a href="{{ esc_url(get_tag_link($tag->term_id)) }}"
               ...
               title="{{ esc_attr(sprintf(_n('%d post', '%d posts', $tag->count, 'sage'), $tag->count)) }}">
              # {{ esc_html($tag->name) }}
            </a>
          </li>
        @endforeach
